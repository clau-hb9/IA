...
public class Node {
	...
	public void printNodeData(int printDebug) {
		...
		for (int i = 0; i < this.herramientas.size(); i++) {
			...
			System.out.println(h.getNombre()+ " " + h.getCantidad());
		}
	...
	public void setHerramienta(int trabajador) {
		...
		for (int i = 0; i < this.herramientas.size(); i++) {				//recorremos las herramientas
			...
			else if(ht!=null&&!h.getNombre().equals(ht.getNombre())&&h.getCantidad()>0&&this.hayTareas(h.getTrabajo())) {  //si es diferente de la que tenia, hay al menos 1 y quedan trabajos por hacer de ese tipo de herramienta
